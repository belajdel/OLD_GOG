...
def index():
    ...
    rendered_template = render_template('index.html', games=games)
    save_index_html(rendered_template)  # Save the rendered template to a file
    return 'index.html saved successfully!'
...
def parse_games(game_files):
    ...
    for file in game_files:
        ...
        game_icon = get_game_icon_from_html(file)
        games.append({'name': game_name, 'link': game_link, 'icon': game_icon})
    return games
...
def save_index_html(rendered_template):
